...
def work_interval(direction, FY1_v, FY1_tFly, FY1_tDrop, dt=0.3):
    """
    返回单枚干扰弹的遮蔽区间 [L, R]，若无遮蔽返回 (-1.0, -1.0)
    采用：粗扫 + 端点二分（减少 time_tick 循环次数）
    """
    # 构造 UAV 投放点 → 烟幕爆点时间 t0
    FY1 = Fly(id=1, direction=direction, v=FY1_v).fly(FY1_tFly).drop(FY1_tDrop)
    t0 = FY1_tFly + FY1_tDrop

    # 可见性判定在“任意时间 t”上的统一封装
    def visible_at(t: float) -> bool:
        M = Missile(id=1).time(t)
        # 烟幕“存在时间”为 [t0, t0+20]；t < t0 时无效
        tau = max(0.0, t - t0)
        if tau > 20.0 + 1e-9:  # 超寿命
            return True  # 视为可见（无遮蔽）
        S = SmokeBall(x=FY1.x, y=FY1.y, z=FY1.z, r=10, v=3).time(tau)
        return Checker.visible(checkCylinder=real, smokeBall=S, missile=M)

    # 1) 粗扫：从 t0 开始，直到 t0+20（寿命）内找到第一处不可见
    #    若一直可见 → 无遮蔽
    tL = t0
    tR = t0 + 20.0
    t = tL
    ...
    while t <= tR + 1e-9:
        if not visible_at(t):
            # 第一次出现不可见（遮蔽）的位置在 [t-dt_coarse, t] 内
            LL = max(tL, t - dt)
            LR = min(tR, t)
            found = True
            break
    ...
    # 2) 左端点二分：在 [LL, LR] 上找最早“不可见”时刻
    while LR - LL > error:
        mid = 0.5 * (LL + LR)
        if not visible_at(mid):
            LR = mid
        else:
            LL = mid
    L = LR

    # 3) 右端点二分：在 [L, tR] 上找最晚“不可见”时刻
    RL, RR = L, tR
    while RR - RL > error:
        mid = 0.5 * (RL + RR)
        if not visible_at(mid):
...
attempts = 0
while nowTime3 < 0 and attempts < 5000:
    direction = random.uniform(0, 2*math.pi)
    FY1_v = random.uniform(70, 140)
    tFly1 = random.uniform(0, INIT_MAX-2.0)
    tFly2 = tFly1 + 1.0 + random.uniform(0, 1.0)
