...
def search_gene_for_fy(FYid):
    Message(f"开始计算FY{FYid}", "INFO")
    for _ in tqdm(range(100), desc=f"FY{FYid}"):
        result = get_new_gene(FYid)
        if result == -1:
            break
        with open(f"data/FY{FYid}_gene.txt", "a") as f:
...
with ThreadPoolExecutor(max_workers=3) as executor:
    futures = [executor.submit(search_gene_for_fy, FYid) for FYid in range(1, 4)]
    for future in as_completed(futures):
        Message(f"FY{future.result()}计算完成", "INFO")
